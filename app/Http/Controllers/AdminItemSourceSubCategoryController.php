<?php namespace App\Http\Controllers;
	...
	class AdminItemSourceSubCategoryController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    */
					...
					$count++;
						...
						try {
							...
							[
								...
								'sub_category_description'   => $value['subcategory_description'],
								'sub_status'                 => $value['subcategory_status'],
		...
		public function getSubCategoryUpdatedDataApi(Request $request) {
			$secretKey = "1f9653c84409990a899f5bd63f719771"; 
            $uniqueString = time(); 
            $userAgent = $_SERVER['HTTP_USER_AGENT']; 
            $userAgent = $_SERVER['HTTP_USER_AGENT']; 
            ...
            //https://stackoverflow.com/questions/8115683/php-curl-custom-headers
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"https://aimfs.digitstrading.ph/public/api/aimfs_sub_category_updated");
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
            curl_setopt($ch, CURLOPT_POST, FALSE);
